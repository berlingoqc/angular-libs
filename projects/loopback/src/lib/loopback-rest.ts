...
export abstract class LoopbackRestClient<T>
  implements LoopbackTableRowDataSource<T> {
  countRoute = 'count';
  baseURL = '';
...
export interface LoopbackTableRowDataSource<T> {
  get: (filter?: Filter) => Observable<T[]>;
...
export class StaticDataSource<T> implements LoopbackTableRowDataSource<T> {
